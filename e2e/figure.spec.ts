...
test.describe('シーンから描いた図', () => {
  test.beforeEach(async ({ page }) => {
    ...
    // ラベルの数式が描画されるまで待つ．
    await expect(page.locator('.figure-label mjx-container')).toHaveCount(4);
  });
  ...
  test('xの名前は，x軸の右の端の，右にある', async ({ page }) => {
    const axis = await box(page.locator('.figure svg path').nth(0));
    const label = await box(page.locator('.figure-label').nth(0));
  ...
  test('yの名前は，y軸の上の端の，上にあり，軸の上に揃う', async ({ page }) => {
    const axis = await box(page.locator('.figure svg path').nth(1));
    const label = await box(page.locator('.figure-label').nth(1));
  ...
  test('Oの名前は，原点の右下に置く', async ({ page }) => {
    const xAxis = await box(page.locator('.figure svg path').nth(0));
    const yAxis = await box(page.locator('.figure svg path').nth(1));
    const origin = {
      x: yAxis.x + yAxis.width / 2,
      y: xAxis.y + xAxis.height / 2,
    };
    const label = await box(page.locator('.figure-label').nth(2));
    // 箱の左上の角が，原点に合う．
  ...
  test('図の実寸は，幅15.2cmで，1cmは，画面の37.8pxである', async ({ page }) => {
    const svg = await box(page.locator('.figure svg'));
  ...
  test('幅の狭い画面では，画面に収まるよう縮み，横にはみ出さない', async ({ page }) => {
    ...
    // 縮んでも，ラベルは，図の中に収まる．
    const frame = await box(page.locator('.figure-frame'));
    const title = await box(page.locator('.figure-label').nth(3));

...
- Samples cover both plane and space figures, reusing the scenes the articles already use (`samples.ts`).
- The interactive viewpoint made one preview do two jobs at once, so it is split into an editable "編集中の図" and a static "プレビュー" that matches the real output exactly (`preview.tsx`, `output-preview.tsx`, `use-figure-renders.ts`). Only the editable one is draggable, and only it shows editing aids: grid and axis objects render in a muted reference style (gray, dotted, thin), and a checkbox adds a wireframe for surface objects, all computed by transforming a copy of the scene (`edit-scene.ts`) rather than by changing the engine, so the real output is unaffected. A formula surface's wireframe is a handful of `curve` objects tracing constant-parameter cross-sections (needs numeric domain bounds, evaluated with a tiny constant-expression parser in `constant-expr.ts` that understands `pi` and `e` but not parameters); a Bézier surface's wireframe is its control net, drawn as invisible `point` objects linked by reference-styled `segment`s. If the transform itself makes the scene fail to render, the editable view falls back to the real output rather than showing an error about an object the person never created.

...
- Points, vectors, segments (`point`, `vector`, `segment`; plane figures only; `compile.rs`, `point_items`, `link_item`): a `point` has `at` (two numbers or expressions), optional `label` (a formula, `anchor` defaults to `south west`, so the name is at the upper right of the point), `dot` (a filled circle of radius 2 pt, IR item `dot`, TikZ `\fill`), and `style.color`. A `vector` (default arrow `stealth`) and a `segment` (no arrow) join two points named by `id`; an unknown id, or the id of something that is not a point, is `unknown_point`. A zero-length link draws nothing. Vectors and segments may come before the points they name, because their end coordinates are looked up after every point is evaluated.
...
- A region can only be hatched, not filled or shaded (the IR has no fill or opacity), and its boundary must be graphs `y = f(x)`, not curves or implicit curves.

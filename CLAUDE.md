...
- `site/src/figure-editor/` (the editor's logic, no React: `draft.ts` scene draft and operations, including `rotateView` for the drag-to-rotate viewpoint, `defaults.json` initial objects per type, `create.ts` `OBJECT_TYPES` and `createObject`, `fields.ts` the input-field table per type, `values.ts` pure helpers that read and write field values, `json.ts` the `Json` type, `edit-scene.ts` `buildEditScene` for the editable view's reference styling and surface wireframes, `constant-expr.ts` a tiny constant-expression parser (`pi`, `e`, arithmetic) used to resolve a wireframe's numeric domain bounds, `samples.ts` (both plane and space figures), `download.ts`, `tikz-document.ts`), `site/src/components/figure-editor/` (the React island: `figure-editor.tsx` shell, `toolbar.tsx`, `form-panel.tsx`, `object-list.tsx`, `field.tsx` with `field-basic.tsx` and `field-special.tsx`, `use-figure-renders.ts` for rendering both the editable and the output scene, `preview.tsx` (editable, draggable, with editing aids) and `output-preview.tsx` (the real output, static) for the two preview panels, `use-drag-rotate.ts` for dragging a space figure's preview to change the viewpoint, `use-draft.ts` for the localStorage autosave), and `site/src/content/docs/figure-editor.mdx` (the public page in the sidebar). The engine validates everything and its rendering is never altered; the editor only builds JSON, including the editable view's aids. The TikZ output does not embed the scene JSON (`crates/figure/src/tikz.rs`), to keep it short; the editor exports both files separately.

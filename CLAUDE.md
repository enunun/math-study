...
- `site/src/content/docs/analysis/`: articles (`continuity.mdx` is the reference article that uses every notation). A new subject directory needs a group in the `sidebar` of `site/astro.config.ts`; since Starlight 0.39 a group is `{ label, items: [{ autogenerate: { directory } }] }`, not `{ label, autogenerate }`. `site/src/content.config.ts`: the `pageId` frontmatter schema. `site/src/content/docs/dev/`: the hidden pages (`notation.mdx` notation reference, `abs.mdx` cross-reference target, `internals.mdx` explanation).

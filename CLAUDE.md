...
- `crates/figure/` (Rust: scene types, `parse_scene`, version policy, validation, `expr/` expression evaluator, `compile.rs`, `derivative.rs` (numerical differentiation, central difference, shared by tangent lines and tangent planes), `sample.rs`, `clip.rs`, `hatch.rs` and `region.rs` for regions, `arrow.rs`, the IR `figure.rs`, `render.rs` (also `tangent_of`/`tangent_line_items` for a plane `tangent_line`), `space.rs` (space rendering, including `wireframe_line` (takes a `default_line: Line`, since a tangent plane's edges default to solid, unlike a dotted wireframe), `surface_wireframe_items`, `control_net_items`, `sphere_wireframe_items`, `tangent_plane_items`), `surface.rs`, `bezier.rs` (Bézier surfaces, `bezier_point`/`casteljau` on `Point3`; and Bézier curves, `bezier_curve_point`/`lerp_point` on dimension-agnostic `Vec<f64>`, shared by plane and space `curve` objects), `spline.rs` (spline curves, `catmull_rom_point`, the same `Vec<f64>` convention as `bezier_curve_point`, plugged into `CurvePlot.spline` next to `CurvePlot.net`), and `refine.rs` (polishing of cuts and intersections) for space figures and surfaces, `tikz.rs`; tests include `tests/wireframe.rs` for `Surface.wireframe`, `Surface.control_net`, and `Sphere.wireframe`, `tests/bezier_curve.rs` for `Curve.bezier`, `tests/spline.rs` for `Curve.spline`, and `tests/tangent.rs` for `TangentLine` and `TangentPlane`), `crates/figure-wasm/` (the wasm wrapper with the typed `SceneOutcome`), `site/src/figure/` (Wasm loader, `svg.ts` and `label.ts` for the SVG and labels, the samples of the check page), `site/src/plugins/rehype-figures.ts` (`<Figure src>` → figure, before `rehypeMathjax`), `site/src/styles/figure.css`, `site/src/content/docs/dev/figure-first.mdx` (the first figure), `site/src/content/docs/dev/figure-space.mdx` (the space figures), `site/src/components/figure/` (the React island), `site/src/projection/` (`camera.ts`: the projection basis in TypeScript, checked against the engine in `camera.test.ts`; `scene.ts`: the scene of the interactive figure) with `site/src/components/projection/` (the sliders island that draws it in the browser through `site/src/figure/hast-react.tsx`, which turns the SVG tree into React elements), `site/src/content/docs/dev/figure-scene.mdx` (the check page), `site/src/content/docs/dev/figure-algorithms.mdx` and `figure-fill.mdx` (the mathematical principles of the algorithms, for readers who do not care about the implementation: projection, hiding, lines from surfaces, approximation, and the hatching of plane regions; update them when an algorithm changes), and `site/src/figures/` (the scene JSON files, one per figure, named after their content). The scene format and the plan are in `docs/graph-design.md`. The first figure and a minimal space figure (a sphere with axes) are drawn end to end; the extension list is in `docs/graph-design.md`. `site/public/schema/scene.schema.json` (served at `/math-study/schema/scene.schema.json`) is the JSON Schema for the scene format, hand-written (not generated from the Rust types) and kept honest by `site/src/figure/scene-schema.test.ts` (ajv, draft 2020-12): every real figure and every default object must validate, and a set of invalid scenes must fail. Update it, and its `description` fields, whenever the scene format changes; a rule the schema cannot express gets a note in the field's own `description` instead ("スキーマでは表せない").
- `site/src/figure-editor/` (the editor's logic, no React: `draft.ts` scene draft and operations, including `rotateView` for the drag-to-rotate viewpoint and `NO_SELECTION`/`indexOfId`, `defaults.json` initial objects per type, `create.ts` `OBJECT_TYPES` and `createObject` (a Bézier surface or curve is listed as its own type, `bezier`/`bezierCurve`, distinguished from the formula `surface`/`curve` by the presence of `bezier` in the object, and mapped back to the `surface`/`curve` `type` on write — see `listedType`/`stemOf`), `fields.ts` the input-field table per type, including the `toggleStyle` kind used by a surface's or sphere's `wireframe` and a Bézier surface's `control_net`, `values.ts` pure helpers that read and write field values, `json.ts` the `Json` type, `edit-scene.ts` `buildEditScene` for the editable view's aids (reference styling and generated ticks only — a surface's or sphere's wireframe, and a Bézier surface's control net, are real engine-rendered scene fields, not editor-only aids, so `edit-scene.ts` does not touch them), `samples.ts` (both plane and space figures), `download.ts`, `tikz-document.ts`), `site/src/components/figure-editor/` (the React island: `figure-editor.tsx` shell, `toolbar.tsx`, `form-panel.tsx`, `object-list.tsx`, `field.tsx` with `field-basic.tsx` and `field-special.tsx`, `style-input.tsx` (`StyleInput` for any `Style`-shaped field by name, and `ToggleStyleInput` for a field that is present only when drawn, such as `wireframe`), `use-figure-renders.ts` for rendering both the editable and the output scene, `preview.tsx` (editable, draggable, with editing aids) and `output-preview.tsx` (the real output, static) for the two preview panels, `use-drag-rotate.ts` for dragging a space figure's preview to change the viewpoint, `use-draft.ts` for the localStorage autosave), and `site/src/content/docs/figure-editor.mdx` (the public page in the sidebar, linking the JSON Schema). The engine validates everything and its rendering is never altered; the editor only builds JSON, including the editable view's aids. The TikZ output does not embed the scene JSON (`crates/figure/src/tikz.rs`), to keep it short; the editor exports both files separately.

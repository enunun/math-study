...
- `crates/figure/` (Rust: the figure scene types, `parse_scene`, the version policy, validation) and `site/src/figures/` (the scene JSON files, one per figure, named after their content). The scene format and the plan are in `docs/graph-design.md`. Nothing is rendered yet, so `internals.mdx` does not describe it; add that when a figure can be shown.

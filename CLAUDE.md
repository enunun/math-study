...
- `crates/figure/` (Rust: the figure scene types, `parse_scene`, the version policy, validation) `crates/figure-wasm/` (the wasm wrapper with the typed `SceneOutcome`), `site/src/figure/` (Wasm loader and the samples of the check page), `site/src/components/figure/` (the React island), `site/src/content/docs/dev/figure-scene.mdx` (the check page), and `site/src/figures/` (the scene JSON files, one per figure, named after their content). The scene format and the plan are in `docs/graph-design.md`. Nothing is rendered yet: the check page only shows how a scene is read and validated.

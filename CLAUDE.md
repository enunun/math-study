...
- `site/src/figure-editor/` (the editor's logic, no React: `draft.ts` scene draft and operations, `defaults.json` initial objects per type, `create.ts` `OBJECT_TYPES` and `createObject`, `fields.ts` the input-field table per type, `values.ts` pure helpers that read and write field values, `json.ts` the `Json` type, `samples.ts`, `download.ts`, `tikz-document.ts`), `site/src/components/figure-editor/` (the React island: `figure-editor.tsx` shell, `toolbar.tsx`, `form-panel.tsx`, `object-list.tsx`, `field.tsx` with `field-basic.tsx` and `field-special.tsx`, `preview.tsx`, `use-draft.ts` for the localStorage autosave), and `site/src/content/docs/figure-editor.mdx` (the public page in the sidebar). The engine validates everything; the editor only builds JSON.
